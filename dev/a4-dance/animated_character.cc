                                          ...
                                          const Matrix4 &view_matrix, const Matrix4 &proj_matrix)
{
    ...
    */
    ...
    //draw the figure's bone
    quick_shapes_.DrawLineSegment(ctm, view_matrix, proj_matrix, Color(0.0,0.0,0.0), Point3(0.0, 0.0, 0.0), Point3(0.0, 0.0, 0.0) + skeleton_.BoneDirectionAndLength(bone_name), 0.01);
    ...
    if (bone_name == "lowerback") {
        ...
        Matrix4 lowerback_scale = Matrix4::Scale(Vector3(0.12, 0.23, 0.12));
        Matrix4 lowerback_translate = Matrix4::Translation(Vector3(-0.2,-0.5,0));
        Matrix4 lowerback_rotate = Matrix4::RotationX(GfxMath::ToRadians(33));
        quick_shapes_.DrawSphere(ctm * lowerback_rotate * lowerback_scale * lowerback_translate , view_matrix, proj_matrix, Color(1.0, 0.0, 0.0));

    }
    if (bone_name == "upperback") {

        Matrix4 upperback_scale = Matrix4::Scale(Vector3(0.1, 0.1, 0.1));
        Matrix4 upperback_translate = Matrix4::Translation(Vector3(0,0.2,0));
        quick_shapes_.DrawSphere(ctm *upperback_scale*upperback_translate , view_matrix, proj_matrix, Color(1.0, 0.0, 0.0));

    }
    if (bone_name == "thorax") {

      Matrix4 upperback_scale = Matrix4::Scale(Vector3(0.1, 0.1, 0.1));
      Matrix4 upperback_translate = Matrix4::Translation(Vector3(0,0.5,0));
      quick_shapes_.DrawSphere(ctm *upperback_scale*upperback_translate , view_matrix, proj_matrix, Color(1.0, 0.0, 0.0));
    ...
    if (bone_name == "head") {
      ...
      Matrix4 head_scale = Matrix4::Scale(Vector3(0.10, 0.16, 0.1));
      Matrix4 head_translate = Matrix4::Translation(Vector3(0,0,0.05));
      Matrix4 head_rotate = Matrix4::RotationX(-GfxMath::ToRadians(30));
      quick_shapes_.DrawSphere(ctm * head_translate * head_rotate * head_scale, view_matrix, proj_matrix, Color(1.0, 0.0, 0.0));

      Matrix4 left_eye = Matrix4::Scale(Vector3(0.02, 0.02, 0.02)) * Matrix4::Translation(Vector3(4,5,3));;
      quick_shapes_.DrawSphere(ctm * left_eye, view_matrix, proj_matrix, Color(0.0, 0.0, 0.0));

      Matrix4 right_eye = Matrix4::Scale(Vector3(0.02, 0.02, 0.02)) * Matrix4::Translation(Vector3(-2,5,4));;
      quick_shapes_.DrawSphere(ctm * right_eye, view_matrix, proj_matrix, Color(0.0, 0.0, 0.0));

      Matrix4 left_tentacle1 = Matrix4::RotationZ(GfxMath::ToRadians(-30)) *
                              Matrix4::Translation(Vector3(-0.02, 0.2, 0)) *
                              Matrix4::Scale(Vector3(0.02, 0.1, 0.02));

      Matrix4 left_tentacle2 = Matrix4::RotationX(GfxMath::ToRadians(60)) *
                              Matrix4::Translation(Vector3(0.12,0.17,-0.23)) *
                              Matrix4::Scale(Vector3(0.02, 0.07, 0.02));

      quick_shapes_.DrawCone(ctm * left_tentacle1,view_matrix, proj_matrix, Color(0.0, 0.0, 0.0) );
      quick_shapes_.DrawCone(ctm * left_tentacle2,view_matrix, proj_matrix, Color(0.0, 0.0, 0.0) );

      Matrix4 right_tentacle1 = Matrix4::RotationZ(GfxMath::ToRadians(30)) *
                              Matrix4::Translation(Vector3(0.02, 0.2, 0)) *
                              Matrix4::Scale(Vector3(0.02, 0.1, 0.02));

      Matrix4 right_tentacle2 = Matrix4::RotationX(GfxMath::ToRadians(60)) *
                              Matrix4::Translation(Vector3(-0.12,0.17,-0.23)) *
                              Matrix4::Scale(Vector3(0.02, 0.07, 0.02));

      quick_shapes_.DrawCone(ctm * right_tentacle1,view_matrix, proj_matrix, Color(0.0, 0.0, 0.0) );
      quick_shapes_.DrawCone(ctm * right_tentacle2,view_matrix, proj_matrix, Color(0.0, 0.0, 0.0) );

      Matrix4 mouth = Matrix4::Translation(Vector3(0,-0.1,0.15)) *
                      Matrix4::Scale(Vector3(0.05, 0.03, 0.03));

      quick_shapes_.DrawSphere(ctm * mouth, view_matrix, proj_matrix, Color(0.0, 0.0, 0.0));
